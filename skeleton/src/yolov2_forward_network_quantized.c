    ...
    int32_t *C, int ldc)
{
    ...
    for (i = 0; i < M; ++i) {
        for (k = 0; k < K; ++k) {
            ...
            //#pragma simd parallel for
            for (j = 0; j < N; ++j) {
                c_tmp[j] += (int32_t)A_PART*B[k*ldb + j]; // [V2] int32_t 캐스팅 (오버플로우 방지)
            }
        }
        for (j = 0; j < N; ++j) {
...
void forward_convolutional_layer_q(network net, layer l, network_state state)
{
    ...
    for (z = 0; z < l.inputs; ++z) {
        ...
        state.input_uint8[z] = max_abs(src, MAX_VAL_8);    // [V2] MAX_VAL_UINT_8(255) → MAX_VAL_8(127)
    }
    ...
    int8_t* b = (int8_t*)state.workspace; // [V2] uint8_t → int8_t (부호 있는 연산)
...
void do_quantization(network net) {
	...
	// TODO
	//{{{		
	// [V2] 튜닝된 multiplier 값 (mAP 55.04% → 79.28%)
